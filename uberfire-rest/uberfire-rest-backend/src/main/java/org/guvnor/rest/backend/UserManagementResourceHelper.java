...
public class UserManagementResourceHelper {
    ...
    private void addToCollection(PermissionCollection pc, ResourceType resourceType, Permission permission) {
        ...
        if (permission.isCreate() != null && permissionValidator.isPermissionAllowed(resourceType, CREATE)) {
            pc.add(permissionManager.createPermission(resourceType, CREATE, permission.isCreate()));
        }
        if (permission.isUpdate() != null && permissionValidator.isPermissionAllowed(resourceType, UPDATE)) {
            pc.add(permissionManager.createPermission(resourceType, UPDATE, permission.isUpdate()));
        }
        if (permission.isDelete() != null && permissionValidator.isPermissionAllowed(resourceType, DELETE)) {
            pc.add(permissionManager.createPermission(resourceType, DELETE, permission.isDelete()));
        }
        if (permission.isBuild() != null && permissionValidator.isPermissionAllowed(resourceType, BUILD)) {
    ...
    private boolean areUsersValid(List<String> users) throws UserNotFoundException {
        if (users != null) {
            try {
    ...
    private boolean areRolesValid(List<String> roles) throws RoleNotFoundException {
        if (roles != null) {
